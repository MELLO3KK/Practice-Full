...
function checkAnswer() {
    ...
    if (!isCorrect) {
        ...
        let insertIndex = -1;
        // Find the index of the first question in the queue that belongs to a different group
        for (let i = 1; i < questionQueue.length; i++) {
            const nextQuestionIndex = questionQueue[i];
            const nextGroup = currentQuiz.questions[nextQuestionIndex].group || "";
        ...
        if (insertIndex === -1) {
            ...
            // Insert before the first question of the next group
