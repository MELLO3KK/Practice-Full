        ...
        currentQuiz.questions.forEach((q, index) => {
            ...
            questionBlock.innerHTML = `
                <div class="question-header">
                    ...
                    <button class="btn-danger btn-delete-question"><i class="fas fa-trash-alt"></i></button>
                </div>
                <input type="text" class="question-text-input" placeholder="Question Text" value="${q.text}">
                <div class="media-upload-container">
                    <div class="media-preview">
                        ${q.media ? `
                            ...
                            <div class="media-info">
                                ...
                                <button class="btn-danger-small btn-remove-media">Remove</button>
                            </div>
                        ` : ''}
                    </div>
                    <div class="media-controls">
                        ...
                        <input type="file" id="media-upload-${index}" class="media-upload-input" accept="image/*,video/*,audio/*" style="display: none">
                    </div>
                </div>
                ...
                <div class="options-grid">
                    ${q.options.map((opt, optIndex) => `
                        <div class="option-row">
                            <input type="radio" class="correct-answer-radio" name="correct_${index}" ${q.correctAnswerIndex === optIndex ? 'checked' : ''} data-opt-index="${optIndex}">
                            <input type="text" class="form-control option-text-input" placeholder="Option ${optIndex + 1}" value="${opt}" data-opt-index="${optIndex}">
                            <button class="btn-danger-small btn-delete-option" data-opt-index="${optIndex}">X</button>
                        </div>
                    `).join('')}
                </div>
            ...
            questionBlock.querySelector('.btn-delete-question').addEventListener('click', () => deleteQuestion(index));
            questionBlock.querySelector('.question-text-input').addEventListener('input', (e) => updateQuestionText(index, e.target.value));

            const removeMediaBtn = questionBlock.querySelector('.btn-remove-media');
            if (removeMediaBtn) {
                removeMediaBtn.addEventListener('click', () => removeMedia(index));
            }

            questionBlock.querySelector('.media-upload-input').addEventListener('change', (e) => handleMediaUpload(index, e.target));

            questionBlock.querySelectorAll('.option-row').forEach(optionRow => {
                const optIndex = parseInt(optionRow.querySelector('.correct-answer-radio').dataset.optIndex, 10);
                optionRow.querySelector('.correct-answer-radio').addEventListener('change', () => updateCorrectAnswer(index, optIndex));
                optionRow.querySelector('.option-text-input').addEventListener('input', (e) => updateOptionText(index, optIndex, e.target.value));
                optionRow.querySelector('.btn-delete-option').addEventListener('click', () => deleteOption(index, optIndex));
            });

            questionBlock.querySelector('.btn-add-option').addEventListener('click', () => addOption(index));
